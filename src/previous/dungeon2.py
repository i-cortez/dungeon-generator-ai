...
class Generator(object):
    ...
    def mutate(self, genome, room_list, corridor_list):
        global HEIGHT, WIDTH
        genome = []
        ...
        return genome, room_list, corridor_list

    def generate_children(self, other):
        new_room_list = copy.deepcopy(self.room_list)

        if len(new_room_list) > 1 and len(other.room_list) > 1:
            split = random.randonint(1, len(new_room_list)-1)
            i = split
            while i < len(self.room_list):
                del new_room_list[random.randonint(0, len(new_room_list)-1)]
                i += 1
            i = split
            while i < len(other.room_list):
                del other.room_list[random.randonint(0, len(other.room_list) - 1)]
                i += 1
            chance = random.random()
            if chance > .5:
                new_room_list += other.room_list
            else:
                new_room_list = other.room_list + new_room_list
        else:
            if len(other.room_list) > 0:
                new_room_list.append(other.room_list[0])
        # do mutation
        corridor_list = []
        return Generator(self.mutate(self.genome, new_room_list, corridor_list))
    ...
    def gen_room(self):
        ...
        tile = random.choice([TILES['floor'], TILES['item'], TILES['enemy'], TILES['ranged'], TILES['key'], TILES['trap']])
    ...
    def room_overlapping(self, room, room_list):
        ...
        for current_room in room_list:
            ...
            if (x < (current_room[0][0] + current_room[0][2]) and current_room[0][0] < (x + w) and y < (current_room[0][1] + current_room[0][3]) and current_room[0][1] < (y + h)):
                return True
        return False
